...
export const trackerConfigs = pgTable("tracker_configs", {
  ...
  lastRefreshStatus: pgEnum("refresh_status", ["idle", "running", "success", "partial", "failed"]).notNull().default("idle"),
...
export const vacancies = pgTable("vacancies", {
  ...
  region: pgEnum("vacancy_region", ["Wrocław onsite/hybrid", "Poland remote", "Europe remote", "Cross-border remote", "Global remote"]).notNull(),
  workModel: varchar("work_model", { length: 255 }).notNull(),
  matchScore: integer("match_score").notNull(),
  freshness: pgEnum("vacancy_freshness", ["Fresh", "Current", "Verify freshness", "Confirm eligibility"]).notNull().default("Current"),
...
export const vacancyPreferences = pgTable("vacancy_preferences", {
  ...
  status: pgEnum("preference_status", ["saved", "hidden"]).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow().notNull(),
}, (table) => [
...
export const refreshRuns = pgTable("refresh_runs", {
  id: integer("id").autoincrement().primaryKey(),
  status: pgEnum("refresh_run_status", ["running", "success", "partial", "failed"]).notNull(),
...
export const manualSearchRequests = pgTable("manual_search_requests", {
  ...
  status: pgEnum("research_request_status", ["requested", "researching", "completed", "failed"]).notNull().default("requested"),
